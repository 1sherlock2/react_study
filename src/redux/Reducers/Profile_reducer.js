  ...
  switch (action.type) {
    case ADD_POST:
      return {
        ...state,
        posts: [...state.posts, {
          id: 4, message: action.post, likeCount: 1, photo: state.profile.photos.small,
        }],
        currentText: null,
      };
...
export const toggleIsFetchingLoad = (isFetching) => ({type: TOGGLE_IS_FETCHING, isFetching});
...
export const profileServerThunk = (userId) => async (dispatch) => {
  ...
  const response = await profileAPI.profileFromServer(userId);
...
export const changeImageThunk = (file) => async (dispatch) => {
  const response = await profileAPI.changeImageFromServer(file);
  if (response.data.resultCode === 0) {
